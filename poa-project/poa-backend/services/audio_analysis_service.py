...
from config import ASR_ENABLED, UPLOAD_DIR

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("audio_analysis")

# 复用 asr_service 的全局模型
...
def _check_whisper() -> bool:
    ...
    if not ASR_ENABLED:
        logger.info("[audio_analysis] ASR_ENABLED=false，音频分析不可用")
    ...
    try:
        ...
        logger.warning("[audio_analysis] openai-whisper 未安装")
        _whisper_available = False
        return False
...
def _load_model() -> Optional[object]:
    ...
    if not _check_whisper():
        return None
    import whisper
    logger.info("[audio_analysis] 加载 Whisper base 模型（音频分析）...")
    _whisper_model = whisper.load_model("base")
    logger.info("[audio_analysis] Whisper 模型就绪")
    return _whisper_model
