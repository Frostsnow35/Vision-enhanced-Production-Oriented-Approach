...
3. goal: 写出可验证的交际目标，必须包含至少2个具体的产出标准（如"用委婉请求句式点单并确认特殊需求"）
4. context_constraints: 至少2条场景特有的限制条件（时间压力/特殊顾客需求/意外情况等）
5. evaluation_criteria: 3~5条评价维度，每条针对特定语言功能（不能全是"准确性""流利度"这种通用维度）
...
{"scene_label":"场景专有名称","poa_task":{"roles":"A:具体角色; B:具体角色","goal":"可验证的交际目标+至少2个产出标准","context_constraints":"场景特有的限制条件","evaluation_criteria":["具体维度1","具体维度2","具体维度3"]},"variant_plot":"不同情节变体描述"}"""
