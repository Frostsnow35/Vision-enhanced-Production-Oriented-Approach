...
export default function FacilitatePage() {
  ...
  const [taskContext, setTaskContext] = useState<{ scene_label: string; roles: string; goal: string }>({
    scene_label: "",
    roles: "",
    goal: "",
  });
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {gaps.map((g, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1 rounded-full bg-red-100 px-4 py-2 text-sm font-medium text-red-700 border border-red-200 hover:bg-red-200 transition-colors"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {weakDims.map((dim) => (
              <div
                key={dim}
                className="rounded-xl border border-red-200 bg-red-50 p-4 hover:shadow-md transition-shadow"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex size-10 items-center justify-center rounded-full bg-red-500 text-white text-lg">
                      !
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-red-900">{dim}</span>
                        <span className="text-xs text-red-600 bg-red-100 px-2 py-0.5 rounded-full">
                          {(scoreMap[dim] ?? 0).toFixed(1)} / 5.0
                        </span>
                      </div>
                      <p className="text-xs text-red-700 mt-1">{DIM_ADVICE[dim]}</p>
                    </div>
                  </div>
                  <button
