...
const SYSTEM_PROMPT = `你是英汉词典工具。你的唯一职责：把用户输入的英文单词翻译成中文，并给出简版音标。
...
export async function translateWord(word: string): Promise<DictResult> {
  ...
  const modelId = process.env.NEXT_PUBLIC_DOUBAO_MODEL_ID;
  const baseUrl = process.env.NEXT_PUBLIC_DOUBAO_BASE_URL || "https://ark.cn-beijing.volces.com/api/v3";
  if (!apiKey || !modelId) {
    return { translation: "（未配置 API Key）", phonetic: "" };
  }
