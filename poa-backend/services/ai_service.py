...
_VISION_TIMEOUT = 120  # 视觉模型超时（Seed 2.0 Mini 多模态 ~3s，留充裕缓冲）
...
_SINGLE_PROMPT = """你是英语口语评估专家。请严格按照以下五维标准评分(1-5,精确到0.1)，并写中文评语。返回 JSON:
{"scores":{"语言能力":0,"话语能力":0,"行动能力":0,"社会文化能力":0,"策略能力":0},
 "comments":{"语言能力":"评语","话语能力":"评语","行动能力":"评语","社会文化能力":"评语","策略能力":"评语"}}

【五维评分锚点 — 必须严格参照】
1.语言能力(发音/语法/词汇): 1=发音错误多,语法错误率>=35%,用词脱节 | 3=发音基本清晰,错误<15%,用词基本匹配 | 5=发音清晰,语法错误<5%,用词高度匹配
2.话语能力(连贯性/话轮/流利度): 1=回应困难,话轮失衡,缺少衔接 | 3=能自然回应,基本连贯 | 5=回应灵活,衔接自然,对话流畅高效
3.行动能力(语言功能/任务完成度/信息完整性): 1=任务完成率<50% | 3=完成率>=70%,意图可理解 | 5=完成率>=85%,高效完成
4.社会文化能力(礼貌表达/场景适配): 1=礼貌表达率<30%,生硬 | 3=礼貌率>=50%,基本得体 | 5=礼貌率>=85%,精准匹配场合
5.策略能力(沟通策略/请求澄清/自我修正): 1=遇阻即中断,缺乏策略 | 3=能基本运用澄清/修正 | 5=策略运用娴熟,沟通顺畅

【评语要求】每个 comment 必须引用对话中的具体证据(原句或描述)，长度 20-60 字。语言能力中的发音和话语能力中的流利度评语直接引用文本证据说明，不提及"无音频"或"由文本推断"。"""

_COMPARE_PROMPT = """你是英语口语评估专家。对比初次和二次对话，对五维逐项打分并写对比评语。返回 JSON:
{"comparison":[
  {"dimension":"语言能力","attempt1_score":2.5,"attempt2_score":3.5,"change":"+1.0",
   "comment":"评语（必须引用两次对话的原文举例，清晰说明分数变化原因）"}]}

【五维评分锚点 — 与单次评估标准完全一致】
1.语言能力(发音/语法/词汇): 1=发音错误多,语法错误率>=35%,用词脱节 | 3=发音基本清晰,错误<15%,用词基本匹配 | 5=发音清晰,语法错误<5%,用词高度匹配
2.话语能力(连贯性/话轮/流利度): 1=回应困难,话轮失衡 | 3=能自然回应,基本连贯 | 5=回应灵活,衔接自然
3.行动能力(语言功能/任务完成度): 1=完成率<50% | 3=>=70% | 5=>=85%,高效完成
4.社会文化能力(礼貌/场景适配): 1=礼貌率<30% | 3=>=50% | 5=>=85%,精准匹配
5.策略能力(沟通策略/澄清/修正): 1=遇阻中断 | 3=基本运用澄清/修正 | 5=策略娴熟,沟通顺畅

【对比评语要求】
每个 comment 必须包含: 1)初次产出中的具体证据(原句) 2)二次产出中的具体证据(原句) 3)进步或退步的具体原因。语言能力中的发音和话语能力中的流利度评语直接引用文本证据说明变化，不提及"无音频"或"由文本推断"。
示例: "初次使用了 'I want coffee'(祈使句直接), 二次改为 'Could I have a latte, please?'(委婉请求+please), 礼貌策略从直接型升级为委婉型,社会文化能力显著提升。"
"""
