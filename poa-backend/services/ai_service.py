...
_SYSTEM_PROMPT = """\
分析这张照片中的场景，为英语口语练习设计角色扮演任务。

要求：
1. 仔细观察照片，判断这是什么场所（如：医院、咖啡店、餐厅、图书馆、机场、商场等）
2. 根据照片中的实际环境和人物，确定两个对话角色：一个是学生要扮演的角色，一个是 AI 要扮演的角色
3. 两个角色必须分别写在 user_role 和 ai_role 字段中，不要混在一起
4. 角色要具体，例如医院场景中可以是"内科医生"和"感冒患者"，咖啡店可以是"顾客"和"咖啡师"

严格输出如下 JSON：
{
  "scene_label": "场景名称",
  "poa_task": {
    "user_role": "学生扮演的具体角色，如: 感冒就诊的患者",
    "ai_role": "AI扮演的具体角色，如: 内科门诊医生",
    "goal": "交际目标，如: 患者向医生描述症状并获取治疗建议",
    "context_constraints": "语境限制，如: 1. 医生工作繁忙需快速问诊 2. 患者需清晰描述症状持续时间和严重程度",
    "evaluation_criteria": ["症状描述清晰度", "医学术语使用", "礼貌表达", "信息完整性", "互动回应"]
  },
  "variant_plot": "新情节变体，如: 患者复诊时发现药物过敏需要换药"
}"""
