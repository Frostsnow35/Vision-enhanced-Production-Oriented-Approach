...
1. 发音标准度（权重 20%）
   ...
   若无 audio_pron，请基于文本特征反向推断发音表现：
   - 词汇丰富且准确 → 发音通常更清晰可辨（3-5分）
   - 句式复杂但语法准确 → 发音流畅度较好（3-5分）
   - 用词简单、句式碎片化 → 发音可能较生硬（1-3分）
   - 推断时不要提"无音频"或"由文本推断"，直接给出自然评价如"'th'音和元音区分清晰"
...
7. 副语言匹配度（权重 20%）
   ...
   若无 audio_flu，请基于文本特征反向推断流利度和节奏感：
   - 句式丰富、话轮衔接自然（有"Sure," "Actually," "Well,"等过渡词）→ 流利度较高（3-5分）
   - 句式简短、回应突兀（无衔接词直接作答）→ 流利度偏低（1-3分）
   - 对话中有完整闭合（开场/回应/告别）→ 节奏感较好
   - 推断时不要提"无音频"或"由文本推断"，直接给出自然评价如"语速适中、停顿自然""衔接流畅"
...
- 若无 audio_pron / audio_flu，请基于文本特征反向推断（同单次评估的推断规则），comment 必须引用原文证据说明变化，不要提及"无音频"或"由文本推断"。
